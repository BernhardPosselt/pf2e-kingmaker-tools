...
* Removed all AI art and made art directory configurable
...
* Army setup was moved from the **Recruit Army** activity to the kingdom creation tab

### Fixed

* It is now possible to have multiple **Recruitable Armies** folders with the same name but different id
...
* Added -5 penalty checkbox toggle for subsisting after 8 hours of exploration
* Reset all camping time trackers after advancing 24 hours or more and not just the one from the active party
...
* Only run migration checks for first GM
...
* Auto calculate total loss in food rather than requiring users to insert it manually
